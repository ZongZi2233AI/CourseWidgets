...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          children: [
            const Text('CourseWidgets', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            if (provider.hasData) ...[
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (result && mounted) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('HTML导入成功！')),
                  );
                }
              }
            },
          ...
          IconButton(
            ...
            onPressed: () async {
              if (!provider.hasData) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('请先导入课表数据')),
                  );
                }
                return;
              }
      ...
      child: Column(
        ...
        children: [
          ...
          TextButton(
            onPressed: () async {
              // 从assets导入测试数据
              final result = await context.read<ScheduleProvider>().importFromAssets();
              if (result && mounted && context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('从assets导入成功！')),
                );
              }
            },
  ...
  /// 显示学期配置弹窗
  void _showSemesterConfigDialog(BuildContext context, ScheduleProvider provider) {
            ...
            children: [
              TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 20), // 增加内边距
                ),
                    ...
                    child: ListTile(
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          if (isActive) const Text('✓ 当前使用', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
                        ],
                      ),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          // 切换按钮
                          IconButton(
                            icon: Icon(Icons.check_circle, color: isActive ? Colors.green : Colors.grey),
                            tooltip: '切换到此记录',
                            onPressed: isActive ? null : () async {
                              ...
                              if (success && context.mounted) {
                                Navigator.pop(context);
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text('已切换到 ${item['name']}')),
                                );
                                // 重新加载数据
                                context.read<ScheduleProvider>().loadSavedData();
                              }
